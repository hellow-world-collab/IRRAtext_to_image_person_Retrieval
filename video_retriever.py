...
class VideoRetriever:
    ...
    def _transcode_to_h264(self, src_path: str) -> str:
        ...
        command = ["ffmpeg", "-i", str(p), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "veryfast", "-y", str(dst_path)]
        try:
    ...
    def search_and_save_top_segments(self, video_path: str, text_query: str, top_n: int = 1, segment_duration: float = 5.0, similarity_threshold: float = 0.2) -> list:
        ...
        for start_frame in range(0, total_frames, frames_per_segment):
            ...
            similarity = (text_embedding @ video_embedding.T).item()
